...
def listener():
    topic = 'ar10/commands'
    print("AR10 ros node has started! listen topic:",topic)
    rospy.init_node('AR10_listener', anonymous=True)  # defines anonymous listener node

    rospy.Subscriber(topic, String, response)  # Subscribes to publisher node 'commands'
...
def response(data):  # main that is called when a message from commands is recieved
    ...
    while x == 1:
        ...
        rospy.loginfo(rospy.get_caller_id() + 'I heard %s',
                      data.data)  # logs messages recieved from commands to /rosout
        option = data.data
